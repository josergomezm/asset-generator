  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div>
      ...
      <textarea
        ...
        placeholder="Describe the image you want to generate (e.g., 'A modern office space with natural lighting')"
      />
    ...
    <div class="border-t border-gray-200 pt-6">
      <div class="flex items-center justify-between mb-4">
        <h4 class="text-md font-medium text-gray-900">Advanced Parameters (Optional)</h4>
        <button
          type="button"
      ...
      <div v-if="showAdvanced" class="grid grid-cols-2 gap-4">
        <div>
          ...
          <select
            id="width"
            v-model="form.generationParameters.width"
            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          >
        ...
        <div>
          ...
          <select
            id="height"
            v-model="form.generationParameters.height"
            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          >
            <option value="512">512px</option>
            <option value="768">768px</option>
        ...
        <div>
          ...
          <input
            id="steps"
            v-model.number="form.generationParameters.steps"
            type="number"
            min="10"
            max="100"
            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
        ...
        <div>
          <label for="guidance" class="block text-sm font-medium text-gray-700">
            Guidance Scale
          </label>
          <input
            id="guidance"
            v-model.number="form.generationParameters.guidance_scale"
            type="number"
            min="1"
            max="20"
            step="0.5"
    ...
    <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
      ...
      >
        ...
        {{ loading ? 'Starting Generation...' : 'Generate Image' }}
      </button>
    </div>
  </form>
...
async function handleSubmit() {
  ...
  try {
    ...
    const result = await apiClient.generateImage(requestData)
    
    // Start monitoring generation status
    generationJob.value = result.job
    startStatusPolling()
    
    showInfo('Generation Started', 'Your image generation has been queued and will begin shortly.')
    ...
    if (error.status === 404) {
      ...
      showError('Generation Failed', error.message || 'Failed to start image generation. Please try again.')
    }
  } finally {
    loading.value = false
  }
    ...
    try {
      ...
      if (generationJob.value.status === 'completed' || generationJob.value.status === 'failed') {
        ...
        if (generationJob.value.status === 'completed') {
          showSuccess('Image Generated!', 'Your image has been generated successfully.')
          ...
          showError('Generation Failed', generationJob.value.errorMessage || 'Image generation failed. Please try again.')
        }
      }
    } catch (error) {
...
function getStatusMessage(): string {
  ...
  switch (generationJob.value.status) {
    case 'queued':
      return 'Your generation request is in the queue and will begin shortly...'
    case 'processing':
      return 'Creating your image with AI. This may take a few moments...'
    case 'completed':
      return 'Your image has been generated successfully!'
    case 'failed':
      return generationJob.value.errorMessage || 'Generation failed. Please try again.'
    default:
      return ''
  }
}
